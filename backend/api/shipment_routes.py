...
async def list_shipments(
    ...
    return {"total": total, "offset": offset, "limit": limit, "items": [ShipmentRead.model_validate(r) for r in rows]}
...
async def get_shipment(
    ...
    row = (await db.execute(select(Shipment).where(Shipment.id == str(shipment_id)))).scalar_one_or_none()
    if not row:
...
async def update_shipment(
    ...
    if row.status == ShipmentStatus.CANCELLED and body.status and body.status != ShipmentStatus.CANCELLED:
...
async def list_carriers(
    ...
    rows = (
        await db.execute(
            select(Carrier)
            .where(Carrier.tenant_id == user.tenant_id)
...
async def get_carrier(
    ...
    row = (await db.execute(select(Carrier).where(Carrier.id == str(carrier_id)))).scalar_one_or_none()
    if not row:
