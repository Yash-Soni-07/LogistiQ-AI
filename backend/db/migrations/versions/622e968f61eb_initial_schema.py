...
def upgrade() -> None:
    ...
    op.create_table('disruption_events',
        sa.Column('id', sa.UUID(), server_default=sa.text('gen_random_uuid()'), nullable=False),
        sa.Column('tenant_id', sa.UUID(), nullable=False),
        sa.Column('type', sa.Enum('weather', 'traffic', 'accident', 'strike', 'natural_disaster', 'security', name='disruptiontype'), nullable=False),
        sa.Column('severity', sa.Enum('low', 'medium', 'high', 'critical', name='disruptionseverity'), server_default=sa.text("'medium'::disruptionseverity"), nullable=False),
        sa.Column('status', sa.String(length=50), server_default=sa.text("'active'::text"), nullable=False),
        sa.Column('center_geom', geoalchemy2.types.Geometry(geometry_type='POINT', srid=4326, dimension=2, from_text='ST_GeomFromEWKT', name='geometry', nullable=False), nullable=False),
    ...
    op.create_table('users',
        ...
        sa.Column('role', sa.Enum('admin', 'manager', 'operator', 'viewer', name='userrole'), server_default=sa.text("'viewer'::userrole"), nullable=False),
    ...
    op.create_table('shipments',
        ...
        sa.Column('status', sa.Enum('pending', 'in_transit', 'delivered', 'delayed', 'cancelled', name='shipmentstatus'), server_default=sa.text("'pending'::shipmentstatus"), nullable=False),
        sa.Column('mode', sa.Enum('road', 'rail', 'air', 'sea', name='shipmentmode'), server_default=sa.text("'road'::shipmentmode"), nullable=False),
    ...
    op.create_index("ix_disruption_events_center_geom", "disruption_events", ["center_geom"], postgresql_using="gist")
