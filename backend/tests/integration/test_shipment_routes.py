...
async def test_create_shipment_success(app_client: AsyncClient):
    ...
    resp = await app_client.post("/shipments", headers={"Authorization": f"Bearer {token}"}, json={
        "origin": "Mumbai",
        "destination": "Delhi",
        "sector": "fmcg",
        "mode": "road",
...
async def test_create_and_list_shipment(app_client: AsyncClient):
    ...
    await app_client.post("/shipments", headers=headers, json={
        "origin": "Chennai",
...
async def test_get_shipment_by_id(app_client: AsyncClient):
    ...
    headers = {"Authorization": f"Bearer {token}"}
    create_resp = await app_client.post("/shipments", headers=headers, json={
        "origin": "Pune",
        "destination": "Nagpur",
        "sector": "pharma",
        "mode": "road",
...
async def test_get_shipment_not_found(app_client: AsyncClient):
    ...
    resp = await app_client.get(f"/shipments/{fake_id}", headers={"Authorization": f"Bearer {token}"})
...
async def test_update_shipment_status(app_client: AsyncClient):
    ...
    ship_id = create_resp.json()["id"]
    resp = await app_client.patch(f"/shipments/{ship_id}", headers=headers, json={"status": "in_transit"})
    assert resp.status_code == 200
...
async def test_filter_shipments_by_status(app_client: AsyncClient):
    ...
    r1 = await app_client.post("/shipments", headers=headers, json={
        "origin": "A", "destination": "B", "sector": "x", "mode": "road"
    })
    r2 = await app_client.post("/shipments", headers=headers, json={
        "origin": "C", "destination": "D", "sector": "x", "mode": "road"
    })
    await app_client.patch(f"/shipments/{r1.json()['id']}", headers=headers, json={"status": "in_transit"})
