...
async def main():
    ...
    async def mock_fetch_rss(url, source):
        if source == "toi":
            return [
                scanner.Article("Huge trucker strike in Mumbai causes port delay", "http://fake.url/1", "toi", "now"),
                scanner.Article("Transport strike in Mumbai affecting goods", "http://fake.url/2", "toi", "now"),
