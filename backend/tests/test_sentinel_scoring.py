...
async def main():
    ...
    async with AsyncSessionLocal() as db:
        res = await db.execute(select(RouteSegment.id, RouteSegment.highway_code, RouteSegment.risk_score))
    ...
    for r in segments_after:
        ...
        print(f"[{r.highway_code}]  prev_risk={old_val:.2f}  new_risk={new_val:.2f}  threshold_tripped={tripped}")
